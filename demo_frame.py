...
def drawBox(frame,obj):
    # Draw a bounding box.
    cv.rectangle(frame, (obj.left, obj.top), (obj.right, obj.bottom), (255, 0, 0),thickness=5)

    label = '%.2f' % obj.confidence

    # Display the label at the top of the bounding box
    labelSize, baseLine = cv.getTextSize(label, cv.FONT_HERSHEY_SIMPLEX, 1, thickness=3)
    top = max(obj.top, labelSize[1])
    cv.putText(frame, label, (obj.left, obj.top), cv.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255),thickness = 3)
    
# ==================================== test case ======================================================
# setting
if __name__ == '__main__':

    #ceate object detector
    obj_detector = ObjDetector("yolov3","weights/yolov3.cfg","weights/yolov3.weights","weights/coco.names") 

    # create lane detector
    lane_detector = lanenet_detector()
    ...
    for i in range(len(frame_list)):
        ...
        frame = cv.imread('data/' +frame_name) # load test image

        objlist = obj_detector.detect(frame) # detect object
        
        _,lanelist = lane_detector.detect_img(img = frame) # detect lane   
    
        # write objlist in csv file
        fieldnames = ['framecout','id','type','left','top','right','bottom','confidence']
        with open('obj_list.csv', mode='a') as csv_file: 
            ...
            for obj in objlist:
                # write csv
                writer.writerow({'framecout':frame_name,'id':obj.id,'type':obj.type,'left':obj.left,'top':obj.top,'right':obj.right,'bottom':obj.bottom,'confidence':obj.confidence})

        #  write lane list to csv to do
        fieldnames = ['framecout','x0','y0','x1','y1']
        with open('lane_list.csv', mode='a') as csv_file:
            writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
            if header2:
                writer.writeheader()
                header2 = False
            for lane in lanelist:
                # write csv
                writer.writerow({'framecout':frame_name,'x0':lane[1][0],'y0':lane[1][1],'x1':lane[1][2],'y1':lane[1][3]})
    ...
    for obj in objlist:
        ...
        drawBox(frame,obj)
        # draw center
        cv.circle(frame, obj.center, radius = 10, color = (0,255,0), thickness = 4)

    # draw lane
    frame2 = cv.resize(frame, (512, 256), interpolation=cv.INTER_LINEAR)
    for line in lanelist:
        xa, ya, xb, yb = line[1]
        cv.line(frame2, (xa, ya), (xb, yb), (0, 0, 255), thickness = 2)
 
    plt.imshow(frame2)
    plt.savefig("data/case1.png")
    plt.show() 
